...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/portfolio/items", async (req, res) => {
    try {
      ...
      const newItem = await storage.createPortfolioItem(itemData);
  ...
  app.patch("/api/portfolio/items/:id", async (req, res) => {
    try {
      ...
      const updateData = insertPortfolioItemSchema.partial().parse(req.body);
  ...
  app.get("/api/reviews", async (req, res) => {
    try {
      ...
      let reviews;
      if (clientId && !isNaN(clientId)) {
        reviews = await storage.getReviewsByClient(clientId);
      } else {
